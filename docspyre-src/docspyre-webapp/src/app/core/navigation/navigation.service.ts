...
export interface NavItem {
  ...
  label: string;
  icon: string;
  /** Absolute router path for the sidebar item. */
  route: string;
}
...
export interface NavCategory {
  id: string;
  label: string;
  /** Absolute base router path for the navbar menu. */
  route: string;
  items: NavItem[];
}

/**
 * Single source of truth for the navbar menus and their sidebar items.
 * Active state is derived from the current router URL so navigation stays
 * in sync with deep links and browser history.
 *
 * Every sidebar icon is unique across all menus.
 */
...
export class NavigationService {
  ...
  readonly categories = signal<NavCategory[]>([
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'uploads', label: 'Uploads', icon: 'bi-cloud-arrow-up', route: '/app/workspace/uploads' },
  ...
  /** Navbar menu that matches the active route, falling back to the first. */
