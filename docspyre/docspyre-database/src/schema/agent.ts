...
/**
 * Persisted manuals and notes generated by the summarizer agent.
 */
export const documentNotes = pgTable(
  'document_notes',
  {
    ...
    format: varchar('format', { length: 32 }).notNull().default('manual'),
  ...
  (table) => ({
    documentIdIdx: index('document_notes_document_id_idx').on(table.documentId),
    userIdIdx: index('document_notes_user_id_idx').on(table.userId),
    createdAtIdx: index('document_notes_created_at_idx').on(table.createdAt),
  }),
);

/**
 * Durable memory snapshot for the summarizer agent to keep in-graph state minimal.
 * The LangGraph state holds contextRef; full memory is fetched from here on demand.
 */
export const summarizerMemory = pgTable(
  'summarizer_memory',
  {
    id: uuid('id').primaryKey().$defaultFn(() => randomUUID()),
    documentId: uuid('document_id')
    ...
    userId: uuid('user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    contextRef: varchar('context_ref', { length: 64 }).notNull(),
    ...
    userItems: jsonb('user_items').notNull().default({}),
  ...
  (table) => ({
    documentIdIdx: index('summarizer_memory_document_id_idx').on(table.documentId),
    contextRefIdx: index('summarizer_memory_context_ref_idx').on(table.contextRef),
  }),
);
